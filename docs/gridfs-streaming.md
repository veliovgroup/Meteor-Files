By default files served from GridFS returned with `200` response code. Which means if you server video/audio/large downloads, file will be entirely read to memory and serve to client. This behavior is not best solution in terms of performance and resource usage.
